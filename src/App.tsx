...
export type ActiveTab = "home" | "library" | "albums" | "artists" | "smart" | "queue" | "equalizer" | "playlists";
...
export default function App() {
  ...
  const {
    songs, setSongs, setPlaylists, setLoading, setScanProgress, addSongs,
  } = useLibraryStore();
  ...
  const playSong = useCallback(async (song: Song) => {
    ...
    try {
      ...
      // Update play count in store
      setSongs(songs.map(s => s.id === song.id ? { ...s, play_count: (s.play_count || 0) + 1 } : s));
  ...
  const handleNext = useCallback(() => {
    const next = nextTrack(); if (next) playSong(next); else setIsPlaying(false);
  }, [nextTrack, playSong]);

  const handlePrev = useCallback(() => {
    if (audioEngine.currentTime > 3) audioEngine.seek(0);
    else { const prev = prevTrack(); if (prev) playSong(prev); }
  }, [prevTrack, playSong]);
  ...
  const handleRating = useCallback(async (songId: number, stars: number) => {
    ...
    const db = await getDb(); await setRating(db, songId, stars);
  }, [songs]);
  ...
  const handleScanFolder = useCallback(async () => {
    ...
    setSongs(updated); setQueue(updated); setScanProgress(null);
  }, []);
  ...
  const handleAddFiles = useCallback(async () => {
    ...
    const db = await getDb();
    const updated = await getAllSongs(db);
    setSongs(updated); setQueue(updated);
  }, []);

  // ── Onboarding complete ─────────────────────────────────────────────────
  const handleOnboardingComplete = useCallback((newSongs: Song[]) => {
    setSongs(newSongs); setQueue(newSongs);
    setOnboarding(false);
    isInitialized.current = true;
  }, []);
  ...
  useKeyboardShortcuts({
    ...
    onFocusSearch: () => { setActiveTab("library"); setTimeout(() => searchInputRef.current?.focus(), 50); },
  });

  // ── Onboarding loading ──────────────────────────────────────────────────
  if (onboarding === null) return (
    <div style={{ height: "100vh", background: "#070710", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ width: 32, height: 32, borderRadius: "50%", border: "3px solid #7C3AED", borderTopColor: "transparent", animation: "spin 0.8s linear infinite" }} />
        ...
        <div className="content">
          ...
          <nav className="tab-nav">
            ...
            <div className="toolbar">
              <SleepTimerButton />
              <button className="icon-btn" onClick={() => isMiniOpen() ? closeMini() : openMini()} title="Mini Player">⬛</button>
          ...
          <div className="tab-content">
            ...
            {activeTab === "library"   && <LibraryView onPlay={song => { setQueue([song]); playSong(song); }} onRating={handleRating} searchRef={searchInputRef} />}
